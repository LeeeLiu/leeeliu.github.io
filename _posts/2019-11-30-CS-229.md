...
1. 两个算法的loss
    ...
    $min _{\theta} C \sum_{i=1}^{m}[y^{(i)} {cost}_{1}(\theta^{T} x^{(i)})+(1-y^{(i)}) cost_{0}(\theta^{T} x^{(i)})]+\frac{1}{2} \sum_{i=1}^{n} \theta_{j}^{2}$
2. 两个算法的决策边界：
...
3. 核函数similarity：解决线性不可分的问题
    ...
    - 核-SVM-最小化目标loss
    $min _{\theta} C \sum_{i=1}^{m}[y^{(i)} {cost}_{1}(\theta^{T} f^{(i)})+(1-y^{(i)}) cost_{0}(\theta^{T} f^{(i)})]+\frac{1}{2} \sum_{i=1}^{n} \theta_{j}^{2}$
