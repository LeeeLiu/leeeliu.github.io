...
1. 两个算法的loss
    ...
    $\min _{\theta} \frac{1}{m}[\sum_{i=1}^{m} y^{(i)}(-\log h_{\theta}(x^{(i)}))+(1-y^{(i)})((-\log (1-h_{\theta}(x^{(i)})))]+\frac{\lambda}{2 m} \sum_{j=1}^{n} \theta_{j}^{2}$
    - SVM的最小化loss：(cost1、cost0是分段函数，是上面2个log项的近似，计算更加高效)
    $\min _{\theta} C \sum_{i=1}^{m}[y^{(i)} \operatorname{cost}_{1}(\theta^{T} x^{(i)})+(1-y^{(i)}) cost_{0}(\theta^{T} x^{(i)})]+\frac{1}{2} \sum_{i=1}^{n} \theta_{j}^{2}$
2. 两个算法的决策边界：
...
3. 核函数similarity：解决线性不可分的问题
    - m个样本$(x^{(1)}, y^{(1)}),(x^{(2)}, y^{(2)}), \ldots,(x^{(m)}, y^{(m)})$，每个样本有m个特征$f^{(i)}=[f_{1}^{(i)}, f_{2}^{(i)}, \ldots, f_{m}^{(i)}]^{T}$，选择m个landmark $l^{(1)}=x^{(1)}, l^{(2)}=x^{(2)}, \ldots, l^{(m)}=x^{(m)}$。
    ...
    - 核-SVM-最小化目标loss
    $\min _{\theta} C \sum_{i=1}^{m}[y^{(i)} \operatorname{cost}_{1}(\theta^{T} f^{(i)})+(1-y^{(i)}) cost_{0}(\theta^{T} f^{(i)})]+\frac{1}{2} \sum_{i=1}^{n} \theta_{j}^{2}$
